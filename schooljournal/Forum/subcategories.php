	...
	<div id="welcome"><p><?php echo "Welcome ".$_SESSION['login_user'].""?></p></div>
		<div id="search-bar">
			<div id="search-form-wrapper">
				<form id="search-form" action="subcategories.php" method="POST">
		...
		<div class="content_add">
			<?php 
				if (isset($_SESSION['login_user'])) {
					echo "<form action='/schooljournal/Forum/newsubcategory.php?cid=".$_GET['cid']."' method='POST'>
						  <p>Add subcategory: </p>
						  <input type='text' id='subcategory_title' name='sub_title' size='100' />
						  <p>Content: </p>
						  <textarea id='content' name='sub_desc'></textarea><br />
						  <input type='submit' value='Add new subcategory' /></form>";
				} else {
					echo "<p>please login first or <a href='/schooljournal/LoginPage.php'>click here</a> to register.</p>";
				}
			?>
		</div>
		<div class="content">
			<h3>Subcategories</h3>
			<?php dispsubcategories($_GET['cid']);?>
		</div>
